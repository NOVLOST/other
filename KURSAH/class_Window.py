...
class Window(): # окно на котом все рисуется
    ...
    def draw_canvas(self,xo,yo,xm,ym,xt,yt,target_index):
        ...
        self.canvas.create_oval(self.width//4, self.width//4,(self.width//4)+self.circle_scale, (self.width//4)+self.circle_scale, fill="black", outline="#004D40")

        # рисуем маяк
        self.canvas.create_oval(((self.width // 4)+dot_xm ) , ((self.width // 4)-dot_ym) ,
        (((self.width // 4) + self.circle_scale)+dot_xm) , (((self.width // 4) + self.circle_scale)-dot_ym) ,
        fill="green", outline="#004D40")

        #рисуем цель
        self.canvas.create_oval(((self.width // 4) + dot_xt) , ((self.width // 4) - dot_yt) ,
                                (((self.width // 4) + self.circle_scale) + dot_xt) , (((self.width // 4) + self.circle_scale) - dot_yt) ,
        ...
        if self.flag_show_name.get() == True:
            self.canvas.create_text ( ( ( self.width // 4 ) + dot_xm  , ( ( self.width // 4 ) - dot_ym ) + 15) ,text="Маяк", fill="#004D40")
            self.canvas.create_text(( ( self.width // 4 ) + dot_xt  , ( ( self.width // 4 ) - dot_yt ) + 25) ,text=f'цель {target_index}', fill="#004D40")



        self.label_scale.place(x=win.get_width() - (win.get_width() // 3) + 150,
